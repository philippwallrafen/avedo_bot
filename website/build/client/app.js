...
async function updatePriorities(list) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return handleUpdateError(updatedPriorities, new Error(`${errorResponse.error}: ${errorResponse.details || "No additional details"}`));
        }
...
function handleUpdateError(updated, error) {
    if (error instanceof Error) {
        console.error(`%c❌ Error updating agent priorities:%c\n  ${error.message}`, "color: #ff3333; font-weight: bold;", "");
    }
    else {
...
async function updateSkills(radio, surname, name) {
    ...
    try {
        ...
        console.log(`%c✅ Server: Skill erfolgreich aktualisiert%c\n\n  👤 Agent: %c${capitalize(surname)}, ${capitalize(name)}%c\n  📞 Neuer Skill: %c${skillType}`, "color: #4caf50; font-weight: bold;", // 🟢 Green for final success
        "", "color: #9c27b0; font-weight: bold;", // 🟣 Purple for agent info
